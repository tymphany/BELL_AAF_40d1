...
/* Increase max. debug blocks in Auraplus P0 in order
 * to match actual pools.
*/
...
#ifdef PMALLOC_STATS
/* Need the recorded 'requested size' */
#ifndef PMALLOC_RECORD_LENGTHS
#define PMALLOC_RECORD_LENGTHS
#endif
#endif
...
#else
/* Byte addressing allowed */
extern uint8 pmalloc_length[PMALLOC_MAX_DEBUG_BLOCKS];
#define PMALLOC_SET_LENGTH(n,s) \
    do \
    { \
         pmalloc_length[n] = (s); \
    } while(0)
#define PMALLOC_GET_LENGTH(n) pmalloc_length[n]
#endif /* __KALIMBA__ */

...
typedef struct IPC_DEBUG_PARTITION_CONFIG {
    IPC_HEADER header;
    debug_partition_config_key key;
    uint32 value;
} IPC_DEBUG_PARTITION_CONFIG;

typedef struct IPC_DEBUG_PARTITION_INFO {
    IPC_HEADER header;
    debug_partition_info_key key;
    uint32 * value;
} IPC_DEBUG_PARTITION_INFO;

typedef struct IPC_DEBUG_PARTITION_ERASE {
    IPC_HEADER header;
} IPC_DEBUG_PARTITION_ERASE;

typedef struct IPC_STREAM_DEBUG_PARTITION_SOURCE {
    IPC_HEADER header;
} IPC_STREAM_DEBUG_PARTITION_SOURCE;
...
typedef struct IPC_QSPI_IDENTITY_REQUEST {
    IPC_HEADER header;
    uint8 * jedec_id;
    uint16 * device_id;
} IPC_QSPI_IDENTITY_REQUEST;

typedef struct IPC_QSPI_U_I_D_REQUEST {
    IPC_HEADER header;
    flash_ops_uid_read_method uid_method;
    uint8 * uid;
} IPC_QSPI_U_I_D_REQUEST;

typedef struct IPC_QSPI_O_T_P_REQUEST {
    IPC_HEADER header;
    flash_ops_otp_read_method otp_method;
    uint32 otp_address;
    uint16 otp_length;
    uint8 * otp;
} IPC_QSPI_O_T_P_REQUEST;
...
typedef struct IPC_VM_ADVERTISE_POWER_SET_MAXIMUM {
    IPC_HEADER header;
    bool override;
    int16 power;
} IPC_VM_ADVERTISE_POWER_SET_MAXIMUM;
...
typedef struct IPC_VM_BD_ADDR_GET_RSSI {
    IPC_HEADER header;
    const TP_BD_ADDR_T * tpaddr;
} IPC_VM_BD_ADDR_GET_RSSI;
...
typedef struct IPC_VM_BD_ADDR_GET_RSSI_RSP {
    IPC_HEADER header;
    bool ret;
    int16 rssi;
} IPC_VM_BD_ADDR_GET_RSSI_RSP;
...
typedef struct IPC_CHARGER_COMMS_TRANSMIT {
    IPC_HEADER header;
    uint16 length;
    uint8 * data;
} IPC_CHARGER_COMMS_TRANSMIT;
...
typedef struct IPC_ACL_RELIABLE_MIRROR_DEBUG_STATISTICS {
    IPC_HEADER header;
    const TP_BD_ADDR_T * tpaddr;
    acl_reliable_mirror_debug_statistics_t * statistics;
} IPC_ACL_RELIABLE_MIRROR_DEBUG_STATISTICS;
...
typedef struct IPC_DEBUG_PARTITION_RESULT_RSP {
    IPC_HEADER header;
    debug_partition_result ret;
} IPC_DEBUG_PARTITION_RESULT_RSP;
...
typedef union IPC_RSP {
    ...
    IPC_DEBUG_PARTITION_RESULT_RSP ipc_debug_partition_result_rsp;
    ...
    IPC_VM_BD_ADDR_GET_RSSI_RSP ipc_vm_bd_addr_get_rssi_rsp;
#endif /* TRAPSET_BLUESTACK */
#if TRAPSET_STREAM

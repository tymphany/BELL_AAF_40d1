...
/**
 *  \brief Return Debug Partition Source Stream
 *         Source stream can be used for reading data stored on the Debug
 *  Partition.
 *         The data is read sequentially from start to end, StreamSize() can
 *         be used to check if there is more data to be read but does not give
 *         exact size of the data. Instead size of data on the Debug Partition
 *         can be ascertained with DebugPartitionInfo() trap.
 *         Once end of the data is reached, the stream has to be closed.
 *         It can be then re-opened to start reading the beginning.
 *         
 *  \return Stream Source for reading data.
 * 
 * \note This trap may NOT be called from a high-priority task handler
 * 
 * \ingroup trapset_debug_partition
 */
Source StreamDebugPartitionSource(void );

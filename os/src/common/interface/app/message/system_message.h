...
#define MESSAGE_BLUESTACK_VSDM_PRIM       (MESSAGE_BLUESTACK_BASE_ + 23) /*!< VSDM primitive from Bluestack. */
...
/*!
  Message to indicate that a new Charger Comms message has been received.
  The message content is a #MessageChargerCommsInd.

  This message is sent to the task registered with MessageChargerCommsTask().
 */
#define MESSAGE_CHARGERCOMMS_IND (SYSTEM_MESSAGE_BASE_ + 82)

/*!
  Report of the previous Charger Comms message that was transmitted using
  ChargerCommsTransmit().
  The message content is a #MessageChargerCommsStatus.

  This message is sent to the task registered with MessageChargerCommsTask().
 */
#define MESSAGE_CHARGERCOMMS_STATUS (SYSTEM_MESSAGE_BASE_ + 83)
...
/*!
  Message type received when Charger comms message has been received.
*/
typedef struct
{
    uint16 length;     /*!< Length of message */
    uint8* data;       /*!< Message data */
    uint32 start_time; /*!< The system time when the first bit of data in the message was received */
    uint32 end_time;   /*!< The system time when the last bit of data in the message was received */
} MessageChargerCommsInd;

/*!
  Message type received when Charger comms status has been received
*/
typedef struct
{
    charger_comms_msg_status status; /*!< Status */
} MessageChargerCommsStatus;

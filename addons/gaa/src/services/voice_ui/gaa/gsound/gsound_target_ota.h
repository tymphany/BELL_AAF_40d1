...
/**
 * Max length in bytes used by the OTA version string.
 * The OTA version string include the terminating NULL
 * character cannot exceed this value.
 */
...
typedef enum {
  ...
  /**
   * OTA state in case a previous OTA session was active and Target has
   * not yet received all data required to apply the new OTA. Target should
   * expect to receive additional OTA Data packets if requesting to resume
   * from this state.
   */
  GSOUND_TARGET_OTA_RESUME_IN_PROGRESS,

  /**
   * OTA state in case a previous OTA session was active and Target
   * has received all data required to apply the new OTA. Target should
   * expect to receive an OTA Apply if requesting to resume from this state.
   */
...
typedef struct {
  /**
   * Identifies the start offset, in bytes, from the start of the binary
   * OTA file for the new OTA session. Expect the byte_offset parameter
   * of the first GSoundTargetOtaData to match this value.
   *
   * Normally, when starting fresh, this value will be set to 0.
   *
   * This value can be >0 if Target has informed GSound to resume
   * a previous OTA session. However, a resume is not always possible - Target
   * must always honor the start_offset in the GSoundTargetOtaBegin call.
   * Refer to the resume_offset member of GSoundOtaSessionSettings for more
   * details.
   */
  uint32_t start_offset;

  /**
   * Total size in bytes the target should expect to receive for the
   * next OTA session. Target must use this value to track when it has
   * received the full firmware image
   */
  int32_t total_size;

  /**
   * Pointer to a NULL terminated ASCII string containing the version
   * number for the upcoming OTA session.
   *
   * Note: Target must make a copy of this string if required to use later.
   * The string will be no larger than GSOUND_TARGET_OTA_VERSION_MAX_LEN
   */
  const char *ota_version;

  /**
   * Used by Target to identify which device in their system
   * the new OTA session is targeting.
   */
...
typedef struct {
  /**
   * Identifies whether or not the Target is currently receiving an OTA
...
typedef struct {
   ...
   *
   * For example, if the user powers off the device in the middle
   * of an OTA session, Target can request to resume at power on.
   */
  GSoundOtaResumeState requested_state;

  /**
   * The offset, in bytes, from the start of the OTA binary to resume
   * the next OTA. Target is responsible for storing this information in
   * non-volatile memory in case of power loss.
   *
...
typedef struct {
  /**
   * Specifies the number of devices in the Target system which require a
   * unique or explicit OTA upgrade.
   *    - Non-TWS: This value should bet set to 1.
   *
   *    - TWS:     The Target can decide if it wants to receive one binary for
   *               both devices, or a separate binary for each device.
   *               Target should also consider if they have other MCUs in the
   *               system which require an update (i.e. case).
   */
...
typedef struct {
   ...
   *
   * Note: It is possible that GSound will call GSoundTargetOtaData(...) with
   * an unexpected byte_offset after this API is called. If Target receives an
   ...
   * packet synchronously from within a call to GSoundTargetOtaData,
   * this callback is not required. In this case, Target should use the ota_done
...
/**
 * Called at the start of a new OTA process when end user has requested
 * to upgrade firmware.
 *
 ...
 *
 * Note: Target must ignore this API and return an error if receiving an OTA from
 * a source other than GSound.
 *
...
/**
 * OTA data has arrived containing a fragment of the firmware image.
 * This function will be called multiple times, once for
 * every fragment of the firmware image. It will be called until all fragments
 * have been transmitted from remote device.
 *
 * Note: Target is responsible for internally tracking the remaining number
 * of bytes. The total number of bytes Target should expect is provided by
 * the GSoundTargetOtaBegin function call.
 *
 ...
 * buffer is flushed to flash and finally verify the integrity of the
 * final image. If the image is invalid for any reason, Target should return
 * an error.
 *
 * Target is also responsible for ensuring the wrong binary was not sent
 * by verifying the final image supports the model of the current device.
 *
 ...
 * device_index provided in the last call to GSoundTargetOtaBegin to route
 * data to correct device in system.
 *
 * Note: Target must ignore this API and return an error if receiving an OTA from
 * a source other than GSound.
 *
...
/**
 * At this point, all data has been transferred and
 * we are ready to reboot into the new firmware image.
 *
 * Target should reboot to use the new image.
 ...
 * also ensure Slave device reboots into new firmware image following the
 * steps above.
 *
 * Note: Target must ignore this API and return an error if receiving an OTA from
 * a source other than GSound.
 *
 * param[in]: device_index  Used by Target to route Apply to the correct device
 *                          in the system.
 *
 */
 ...
 *
 * Target is responsible for storing resuming information in non-volatile
 * memory in case of power loss.
 *
 * Note, a resume is not always possible. If Target requests to resume
 * an OTA they can still receive a GSoundTargetOtaBegin with offset = 0. In
 * this case OTA must start over.
 *
 ...
 *                                details.
 * param[in]: device_index:       Identifies which device in the Target system
 *                                GSound is requesting resume information from.
 */
 ...
 * Called at boot-up.
 *
 * Target can use this function call to initialize
 * any internal variables required to process a future OTA.
 *

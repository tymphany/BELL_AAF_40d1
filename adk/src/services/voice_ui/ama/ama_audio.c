...
static bool amaAudio_SendOpusVoiceData(Source source)
{
    ...
    AMA_INFO(("amaSendOpusSourceSpeechData on %d transport\n", transport));
    ...
    switch(AmaSpeech_GetAudioFormat())
    {
        ...
        case AUDIO_FORMAT__MSBC:
            default :
                AMA_INFO(("Not Expected Codec on \n"));
                Panic();
                break;
    }
    ...
    AMA_INFO(("lengthSourceThreshold = %d\n", lengthSourceThreshold));
...
/***************************************************************************/
static void amaAudio_HandleVoiceData(Source src)
{
    if(AmaData_IsSendingVoiceData())
    {
        (*amaAudio_SendVoiceData)(src);
    }
    else
    {

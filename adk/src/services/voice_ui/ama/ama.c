...
static void ama_DeselectVoiceAssistant(void)
{
    AMA_INFO(("ama_DeselectVoiceAssistant"));
    AmaSendCommand_NotifyDeviceConfig(ASSISTANT_OVERRIDE_REQUIRED);
}

static void ama_SelectVoiceAssistant(void)
{
    AMA_INFO(("ama_SelectVoiceAssistant"));
...
{
    AMA_INFO(("Ama_GetVoiceUiProtectedInterface"));
...
{
    AMA_INFO(("ama_HandleSwitchTransportInd transport=%u", transport));
    ...
    switch(transport)
    {
        case ama_transport_ble:
            AMA_INFO(("ama_transport_ble with OPUS codec\n"));
            ama_ConfigureCodec(ama_codec_opus);
            break;

        case ama_transport_rfcomm:
            AMA_INFO(("ama_transport_rfcomm with (%u) codec (1->mSBC, 2->OPUS)\n", (ama_codec_t)AMA_DEFAULT_CODEC_OVER_RFCOMM));
        ...
        case ama_transport_iap:
            AMA_INFO(("ama_transport_iap with (%u) codec (1->mSBC, 2->OPUS)\n", (ama_codec_t)AMA_DEFAULT_CODEC_OVER_IAP2));
...
static void ama_MessageHandler(Task task, MessageId id, Message message)
{
    ...
    switch(id)
    {
        ...
        default:
            AMA_INFO(("ama_MessageHandler: unhandled 0x%04X\n", id));
            break;
    }
...
static void ama_EventHandler(ui_input_t   event_id)
{
    AMA_INFO(("ama_EventHandler, event_id %u\n", event_id));
    if (!AmaActions_HandleVaEvent(event_id))
        AMA_INFO(("ama_EventHandler unhandled event %u\n", event_id));    
...
{
    AMA_INFO(("AMA Transport Disconnect"));

...
bool UpgradeFlowControlProcessDataRequest(uint8 *data,
                                                            uint16 size_data)
{
    PRINT(("UPGRADE: UpgradeFlowControlProcessDataRequest\n"));
...
DESCRIPTION
    Get the is_out_case_dfu variable information by reading from UPGRADE_LIB_PSKEY.
*/
bool UpgradeIsOutCaseDFU(void)
{
    bool is_out_of_case_dfu;
    /* To do: Currently, UpgradeCtxGet() can cause panic if it is set as NULL
     * while trying to access it. This UpgradeCtxGet() routine need to be
     * modified to not cause panic, if UpgradeCtx is found to be NULL.
     */
    is_out_of_case_dfu = UpgradeCtxGetPSKeys()->is_out_case_dfu;
    DEBUG_LOG("UpgradeIsOutCaseDFU is_out_of_case_dfu %d", is_out_of_case_dfu);

    return is_out_of_case_dfu;
}

/***************************************************************************
NAME
    UpgradeSetIsOutCaseDFU

DESCRIPTION
    Set the is_out_case_dfu variable information in UPGRADE_LIB_PSKEY.
*/
void UpgradeSetIsOutCaseDFU(bool is_out_of_case)
{
     ...
     * modified to not cause panic, if UpgradeCtx is found to be NULL.
     */
    DEBUG_LOG("UpgradeSetIsOutCaseDFU is_out_of_case %d", is_out_of_case);
    UpgradeCtxGetPSKeys()->is_out_case_dfu = is_out_of_case;
    UpgradeSavePSKeys();
}
...
/***************************************************************************
NAME
    UpgradeIsPriRebootDone

DESCRIPTION
    Check if Primary reboot phase of DFU is completed or not.
RETURN
    Returns pointer to uint16.
...
{
    return &(UpgradeCtxGet()->dfuPriRebootDone);
}

/****************************************************************************
NAME
    UpgradeSetPriRebootDone

DESCRIPTION
    Indicate Primary reboot phase of DFU is completed.

PARAM
    val boolean to indicate reboot is completed or not.
...
{
    /*
     * Flip the bool as it controls a conditionally posted message.
     * val = FALSE, dfuPriRebootDone = 1dfuPriRebootDone = 1 i.e. reboot not completed
     * val = TRUE, dfuPriRebootDone = 0 i.e. reboot is completed.
     */
    UpgradeCtxGet()->dfuPriRebootDone = !val;
    DEBUG_LOG("UpgradeSetPriRebootDone dfuPriRebootDone : %u", val);
}

/* before reboot send error message */
void UpgradeSendError(uint16 errorCode)
{
    UpgradeHostIFDataSendErrorInd(errorCode);
}

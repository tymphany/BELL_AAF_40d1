...
#define GAIA_COMMAND_VM_UPGRADE_FW_NOTIFICATION_TO_APP (0x0C05)
#define GAIA_COMMAND_VM_UPGRADE_FW_DATA_CONFIRM_TO_APP (0x0C06)
 ...
 */
typedef void (*gaia_v3_command_handler_fn_t)(Task task, const GAIA_UNHANDLED_COMMAND_IND_T *command);
...
/*! @brief Connect a GAIA transport to the connection library

    @retval True if the trasport mechanism is valid otherwise send error with GAIA_STATUS_INCORRECT_STATE
*/
bool GaiaUpgradeConnect(void);

/*! @brief Disconnect a GAIA transport from the connection library

    @retval True if the trasport mechanism is valid otherwise send error with GAIA_STATUS_INCORRECT_STATE
*/
bool GaiaUpgradeDisconnect(void);
...
/*! @brief Tunnel a VM Upgrade Protocol command to the upgrade library
    ...
    @retval True if the trasport mechanism is valid otherwise send error with GAIA_STATUS_INCORRECT_STATE
*/
bool GaiaUpgradeControl(uint8 *payload, uint8 payload_length);

/*! @brief Gets the data endpoint mode

    @retval Data endpoint mode
*/
uint8 GaiaGetDataEndpointMode(void);
...
*/
bool GaiaSetDataEndpointMode(uint8 mode);

/*! @brief Sets a command handler to be used from the V3 app

    @param gaia_v3_command_handler Command handler
*/
void GaiaSetV3AppCommandHandler(gaia_v3_command_handler_fn_t gaia_v3_command_handler);

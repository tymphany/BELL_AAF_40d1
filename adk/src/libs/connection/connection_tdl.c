...
static void delete_from_security_manager(uint16 pos)
{
    ...
    if (PsRetrieve(TRUSTED_DEVICE_LIST + pos,  NULL, 0))
    {
        ...
        MAKE_CL_MESSAGE(CL_SM_AUTH_DEVICE_DELETED_IND);
        ...
        VmSendDmPrim(prim);
        MessageSend(connectionGetAppTask(), CL_SM_AUTH_DEVICE_DELETED_IND, message);
    } 
}
        ...
        )
{
    ...
    /* The expected condition is that there is no entry for this device,
     * but the tdl_find_device() function will also provide the next
     * position in the list to add link keys, and its index.
     */
    td = tdl_find_device(req->taddr.type, &req->taddr.addr, &pos, &idx);
    if (!td && req->size_data)
    {
        /* Copy the 'td' data to avoid a memory panic with the PsStore()
         * function call.
         */
        td = (td_data_t *) PanicUnlessMalloc(SIZE_TD_DATA_T);
        memmove(td, req->data, req->size_data * sizeof(uint16));

        /* Delete any existing link keys that may be in the available
         * position.
         */
        delete_from_security_manager(pos);
        ...
        /* The passed in raw link key data should be in the td_data_t
         * structure, so map the td pointer to it and store it.
         */
        if  (
             PsStore(
                 TRUSTED_DEVICE_LIST + pos,
                 td,
                 PS_SIZE_ADJ(CALC_TD_SIZE(td))
                 )
             )
        {
            /* Set hash of new device for more efficient list searches */
            GET_TDI_CACHE.element[idx].hash = TDI_HASH(req->taddr.addr);
            ...
            /*
                ...
                device. Otherwise the sorting wouldn't happen.
            */
            result = ConnectionAuthSetPriorityDevice(&req->taddr.addr, FALSE);

            /* Update the device in the Bluestack Security Manager database */
            dm_sm_add_device_req(td);

            /* Since td is pointing to the message data, we don't have to free td
             * here. It will be freed by the message handler.
             */
        }
        else
        {
    ...
    {
        /* We found  the addres, so we have to free td. */
        result = FALSE;
    }
    free(td);
    return result;
        ...
        )
{
    ...
    *data_len = 0;
    ...
    if (td != NULL)
    {
        *data_len = CALC_TD_SIZE(td);
        *data_len = *data_len / sizeof(uint16);

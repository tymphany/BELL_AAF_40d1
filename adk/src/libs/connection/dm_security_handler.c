...
/****************************************************************************
NAME
    handleAddAuthDeviceRawReq

DESCRIPTION
    This function is called on receipt of an
    CL_INTERNAL_SM_ADD_AUTH_DEVICE_RAW_REQ to add a device to the trusted
    device list.

RETURNS

*/
void handleAddAuthDeviceRawReq(
        connectionSmState *smState,
        ...
        )
{
    if (smState->deviceReqLock)
    {
        size_t data_size = sizeof(uint16)  * req->size_data;
        MAKE_CL_MESSAGE_WITH_LEN(
                    CL_INTERNAL_SM_ADD_AUTH_DEVICE_RAW_REQ,
                    data_size
                    );

        COPY_CL_MESSAGE_WITH_LEN(
                    CL_INTERNAL_SM_ADD_AUTH_DEVICE_RAW_REQ,
                    data_size,
                    req,
                    message
    ...
    {
        /* Add device, check if failed locally */
        if  ( !connectionAuthAddDeviceRaw(req) )
        {
            CL_SM_ADD_AUTH_DEVICE_CFM_T *message =
                PanicUnlessNew(CL_SM_ADD_AUTH_DEVICE_CFM_T);
            message->bd_addr = req->taddr.addr;
            message->status = fail;
            MessageSend(req->theAppTask, CL_SM_ADD_AUTH_DEVICE_CFM, message);
        }
        else
...
/****************************************************************************
NAME
    handleGetAuthDeviceRawReq

DESCRIPTION
    This function is call on receipt of the
    CL_INTERNAL_SM_GET_AUTH_DEVICE_RAW_REQ to get all the link keys
    associated with the identified peer device from the trusted device list.

RETURNS void
*/
void handleGetAuthDeviceRawReq(
        connectionSmState *smState,
        const CL_INTERNAL_SM_GET_AUTH_DEVICE_RAW_REQ_T *req
        )
{
    if (smState->deviceReqLock)
    {
        MAKE_CL_MESSAGE(CL_INTERNAL_SM_GET_AUTH_DEVICE_RAW_REQ);
    ...
    {
        uint16 data_len = 0;
        uint16 *data_ptr = connectionAuthGetDeviceRaw(&req->taddr,  &data_len);
        size_t data_size = data_len * sizeof(uint16);

        { /* message variable scope */
            MAKE_CL_MESSAGE_WITH_LEN(
                        CL_SM_GET_AUTH_DEVICE_RAW_CFM,
                        data_size
                        );
            message->peer_taddr = req->taddr;
            message->size_data = data_len;

            /* if data_len is 0, then no device with that address was found. */
            if (data_len)
            {
                memmove(message->data, data_ptr, data_size);
                message->status = success;

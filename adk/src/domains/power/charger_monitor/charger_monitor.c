...
static void appChargerCheck(void)
{
    ...
    {
        DEBUG_LOG("appChargerCheck, status %u,%u, connected %u,%u, charging %u",
    ...
    if (theCharger->status != ChargerStatus())
    {
        ...
        switch (theCharger->status)
        {
            ...
            case HEADROOM_ERROR:
                ...
                break;
            default:
                DEBUG_LOG("appChargerCheck, status %d", ChargerStatus());
                break;
        }
...
{
    chargerTaskData *theCharger = appGetCharger();
    DEBUG_LOG("appChargerHandleChargeTimeout, charger_status %d", theCharger->status);

...
static void kymera_ConnectChainToMics(const va_mic_chain_op_params_t *params)
{
    ...
    if (mic1_input)
    {
        mic1 = Kymera_GetMicrophoneSource(microphone_1, mic1, params->mic_sample_rate, high_priority_user);
    }
    if (mic2_input)
    {
        mic2 = Kymera_GetMicrophoneSource(microphone_2, mic1, params->mic_sample_rate, high_priority_user);
    }
...
static void kymera_RunUsingConnectedMics(SourceFunction function)
{
    ...
    {
        mics[number_of_mics] = PanicNull(Microphones_GetMicrophoneSource(microphone_1));
        number_of_mics++;
    }

    if (kymera_GetChainInput(EPR_VA_MIC_MIC2_IN))
    {
        mics[number_of_mics] = PanicNull(Microphones_GetMicrophoneSource(microphone_2));
        number_of_mics++;
    }

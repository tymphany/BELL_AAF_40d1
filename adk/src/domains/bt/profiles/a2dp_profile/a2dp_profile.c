...
{
    avInstanceTaskData **instpp, *instp = NULL;
    ARRAY_FOREACH(instpp, AvGetTaskData()->av_inst)
    {
        if (*instpp && appDeviceTypeIsHandset(&(*instpp)->bd_addr))
        {
            instp = *instpp;
            break;
        }
    }
    DEBUG_LOG("A2dpProfile_GetHandsetData(%p)", instp);
    return instp ? &instp->a2dp : NULL;
}
...
static void appA2dpEnterConnectingMediaLocal(avInstanceTaskData *theInst)
{
    ...
    else
    {
        ...
        {
            const uint8 sink_seids[] = {AV_SEID_APTX_ADAPTIVE_SNK, AV_SEID_APTX_SNK, AV_SEID_AAC_SNK, AV_SEID_SBC_SNK};
            A2dpMediaOpenRequest(theInst->a2dp.device_id, ARRAY_DIM(sink_seids), (uint8 *)sink_seids);
        }
    }
}
...
void appA2dpError(avInstanceTaskData *theInst, MessageId id, Message message)
{
    ...
    DEBUG_LOG("appA2dpError(%p), state(0x%x), id(0x%x)", (void *)theInst, theInst->a2dp.state, id);
#endif

...
{
    UNUSED(theInst);
    DEBUG_LOG("appAvrcpEnterConnectingLocalWaitResponse(%p)", (void *)theInst);
}
...
{
    UNUSED(theInst);
    DEBUG_LOG("appAvrcpExitConnectingLocalWaitResponse(%p)", (void *)theInst);
}
...
{
    UNUSED(theInst);
    DEBUG_LOG("appAvrcpEnterConnectingRemoteWaitResponse(%p)", (void *)theInst);
}
...
static void appAvrcpSetState(avInstanceTaskData *theInst, avAvrcpState avrcp_state)
{
    ...
    DEBUG_LOG("appAvrcpSetState(%p) state %d", (void *)theInst, avrcp_state);
...
#ifdef AV_DEBUG
    DEBUG_LOG("appAvrcpError(%p) state %u, id %x", (void *)theInst, theInst->avrcp.state, id);
#else
    Panic();
#endif
}
...
{
    DEBUG_LOG("appAvrcpHandleInternalAvrcpRemoteRequest(%p)", (void *)theInst);
...
{
    DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectRequest(%p) %x,%x,%lx, clients %u, locks %u state 0x%x",
                 (void *)theInst,
    ...
    switch (appAvrcpGetState(theInst))
    {
        case AVRCP_STATE_DISCONNECTED:
        {
            ...
            if (ConManagerIsConnected(&theInst->bd_addr))
            {
                ...
                DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectRequest(%p) added client %p, clients %u, locks %u",
                             (void *)theInst,
                             req->client_task,
                             TaskList_Size(theInst->avrcp.client_list),
                             theInst->avrcp.client_lock);
        ...
        case AVRCP_STATE_CONNECTED:
        {
            ...
            DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectRequest(%p) connected - added client %p, clients %u, locks %u",
                         (void *)theInst,
                         req->client_task,
                         TaskList_Size(theInst->avrcp.client_list),
                         theInst->avrcp.client_lock);
        ...
        case AVRCP_STATE_CONNECTING_REMOTE_WAIT_RESPONSE:
        {
            ...
            DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectRequest(%p) connecting - added client %p, clients %u, locks %u",
                         (void *)theInst,
                         req->client_task,
                         TaskList_Size(theInst->avrcp.client_list),
                         theInst->avrcp.client_lock);
...
static void appAvrcpHandleInternalAvrcpConnectLaterRequest(avInstanceTaskData *theInst)
{
    ...
    DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectLaterRequest(%p)", theInst);
}
...
static void appAvrcpHandleInternalAvrcpDisconnectLaterRequest(avInstanceTaskData *theInst)
{
    ...
    DEBUG_LOG("appAvrcpHandleInternalAvrcpDisconnectLaterRequest(%p)", theInst);
}
...
{
    DEBUG_LOG("appAvrcpHandleInternalAvrcpDisconnectRequest(%p) removing client %p, clients %u, locks %u",
               (void *)theInst,
               req->client_task,
               TaskList_Size(theInst->avrcp.client_list),
               theInst->avrcp.client_lock);
    ...
    {
        theInst->avrcp.client_lock -= 1;
        DEBUG_LOG("appAvrcpHandleInternalAvrcpDisconnectRequest(%p) removed client %p, clients %u, locks %u",
                   (void *)theInst,
                   req->client_task,
                   TaskList_Size(theInst->avrcp.client_list),
                   theInst->avrcp.client_lock);
        ...
        DEBUG_LOG("appAvrcpHandleInternalAvrcpDisconnectRequest(%p) client %p not found",
                   (void *)theInst,
                   req->client_task);
                                                         ...
                                                         const AV_INTERNAL_AVRCP_CONNECT_IND_T *ind)
{
    ...
    DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectIndication(%p) clients %d",
                 (void *)theInst,
                 TaskList_Size(theInst->avrcp.client_list));
...
{
    assert(theInst->avrcp.avrcp == NULL);
    DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectResponse(%p), accept %d", (void *)theInst, res->accept);
    ...
    switch (appAvrcpGetState(theInst))
    {
        case AVRCP_STATE_CONNECTING_LOCAL_WAIT_RESPONSE:
        {
            ...
            {
                DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectResponse(%p), removing %p", (void *)theInst, res->ind_task);
                DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectResponse(%p), removing %p", (void *)theInst, res->client_task);
            ...
            DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectResponse(%p), locks %u, responses_left %u",
            ...
            {
                DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectResponse(%p), connect request", (void *)theInst);
        ...
        case AVRCP_STATE_CONNECTING_REMOTE_WAIT_RESPONSE:
        {
            ...
            if (theInst->avrcp.client_responses == 0)
            {
                DEBUG_LOG("appAvrcpHandleInternalAvrcpConnectResponse(%p), connect response", (void *)theInst);
                                                                   ...
                                                                   AV_INTERNAL_AVRCP_NOTIFICATION_REGISTER_REQ_T *req)
{
    switch (appAvrcpGetState(theInst))
    {
        case AVRCP_STATE_CONNECTED:
        {
            DEBUG_LOG("appAvrcpHandleInternalAvrcpNotificationRegisterRequest(%p), event %d", theInst, req->event_id);
...
{
    DEBUG_LOG("appAvrcpHandleInternalAvrcpPauseRequest(%p)", theInst);
    if (appAvrcpIsPlaybackLocked(theInst))
    {
        /* Re-send message if lock set, as it may be delayed message */
...
{
    assert(theInst->avrcp.avrcp == NULL);
    DEBUG_LOG("appAvrcpHandleAvrcpConnectConfirm(%p) status %d", (void *)theInst, cfm->status);
    ...
    switch (appAvrcpGetState(theInst))
    {
        ...
        case AVRCP_STATE_CONNECTING_REMOTE:
        {
            ...
            {
                /* Notify the profile manager that the connect request failed. */
                appAvAvrcpSendConnectCfmProfileManager(theInst, FALSE);
...
{
    DEBUG_LOG("appAvrcpHandleAvrcpDisconnectIndication(%p)", (void *)theInst);
...
static void appAvrcpHandleRegisterNotificationInd(avInstanceTaskData *theInst, AVRCP_REGISTER_NOTIFICATION_IND_T *ind)
{
    ...
    switch (ind->event_id)
    {
        case avrcp_event_volume_changed:
            DEBUG_LOG("appAvrcpHandleRegisterNotificationInd(%p), volume %u", (void *)theInst, appAvVolumeGet());
...
static void appAvrcpHandleSetAbsoluteVolumeInd(avInstanceTaskData *theInst, AVRCP_SET_ABSOLUTE_VOLUME_IND_T *ind, bool send_response)
{
    ...
    {
        DEBUG_LOG("appAvrcpHandleSetAbsoluteVolumeInd(%p) suppressed, overriding response with %u", (void *)theInst, AudioSources_GetVolume(audio_source_a2dp_1).value);
        if(send_response)
        {
...
static bool appAvrcpClearNotificationLock(avInstanceTaskData *theInst, avrcp_supported_events event_id)
{
    ...
    {
        DEBUG_LOG("appAvrcpClearNotificationLock(%p), clear lock for event %d", theInst, event_id);
        theInst->avrcp.notification_lock = 0;
        return TRUE;
    }
    else
    {
        DEBUG_LOG("appAvrcpClearNotificationLock(%p), no lock to clear for event %d, notification-lock %d", theInst, event_id, theInst->avrcp.notification_lock);
        return FALSE;
    }
}
...
static void appAvrcpHandleRegisterNotificationCfm(avInstanceTaskData *theInst, AVRCP_REGISTER_NOTIFICATION_CFM_T *cfm)
{
    ...
    {
        DEBUG_LOG("appAvrcpHandleRegisterNotificationCfm(%p) event %d is already registered",
                    (void *)theInst, cfm->event_id);
    }
    else
    {
        DEBUG_LOG("appAvrcpHandleRegisterNotificationCfm(%p) status %d, event %d",
...
{
    assert(theInst->avrcp.avrcp == ind->avrcp);
    DEBUG_LOG("appAvrcpHandleEventPlaybackStatusChangedInd(%p), response %u, play status %u",

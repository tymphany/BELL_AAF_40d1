...
/**
 * Function handles transitions while there are pending state change
 * (other transitions).
 * Input parameters
 * splitter - Pointer to splitter operator.
 * streams - bitfield holding the instructions for the streams.
 * instr_stream_set_one - Instruction when bitfield is set to one.
 * instr_stream_set_zero - Instruction when bitfield is set to zero.
 * Instructions are decoded using SPLITTER_OUTPUT_STATE and it translates to:
 *  - Activate = ACTIVE
 *  - Deactivate = INACTIVE
                    ...
                    SPLITTER_OUTPUT_STATE instr_stream_set_one, SPLITTER_OUTPUT_STATE instr_stream_set_zero)
{
    ...
    /* Note: The operator is in transition (splitter->in_transition == TRUE).
     * This means that for at least one stream the current state will be
     * different to the next state.
     * The following states are supported: 0 INACTIVE, 1 ACTIVE and 2 HOLD.
     *
     * The following transitions (current state different to the next state)
     * are instant:
     * - instant transition to INACTIVE
     * - instant transition to HOLD
     * The only transition which can cause a transitional state is:
    ...
    for (index=0; index < SPLITTER_MAX_OUTPUTS_PER_CHANNEL;index++)
    {
        ...
        next = get_next_output_state(splitter, index);
        ...
        {
            /**
             * The current stream needs to be activated.
             * States:
             * (current == INACTIVE) && (next == INACTIVE) - Not supported during transition.
             * (current == ACTIVE  ) && (next == ACTIVE  ) - Nothing to do.
             * (current == HOLD    ) && (next == HOLD    ) - Not supported during transition.
             * (current == HOLD    ) && (next == ACTIVE  ) - Possible transition. Nothing to do, the previous
             *                                               activate didn't finish anyways.
             * (current == INACTIVE) && (next == ACTIVE  ) - Possible transition. Nothing to do, the previous
             *                                               activate didn't finish anyways.
        ...
        {
            /**
             * The current stream needs to be deactivated.
             * States:
             * (current == INACTIVE) && (next == INACTIVE) - Nothing to do.
             * (current == ACTIVE  ) && (next == ACTIVE  ) - Deactivate (transition instant). This in not a revert
             *                                               but it is possible why not allow it.
             * (current == HOLD    ) && (next == HOLD    ) - Deactivate (transition instant). This in not a revert
             *                                               but it is possible why not allow it.
             * (current == HOLD    ) && (next == ACTIVE  ) - Possible transition. Deactivate (transition instant).

...
static void pack_to_internal(SPLITTER_OP_DATA *splitter, unsigned data_to_pack, bool offload_enabled, INT_OP_ID opid)
{
    ...
    if (offload_enabled && audio_thread_offload_is_active())
    {
        ...
        /* Offload the following load: pack_to_internal_data_buffer(splitter, data_to_pack); */
        ...
        if (splitter->offload_enabled)
        {
            if (!splitter->next_run_unpack)
            {
                ...
                if (data_to_pack != 0)
                {
                    ...
                    pack_to_internal(splitter, data_to_pack, TRUE, opid);
                }

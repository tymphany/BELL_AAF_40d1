...
/**
 * The numeric representation of a operator as exposed outside of the audio
 * subsystem. Only the 16 lower bits are used. It can be cast to KYMERA_ID.
 *
 * This is the type that should be used for operator ids seen from outside
 * of the subsystem and from the inside the capability for example when
 * dealing with the operation message client.
 */
typedef unsigned OPERATOR_ID;
#define EXT_OP_ID OPERATOR_ID

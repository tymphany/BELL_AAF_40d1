...
typedef struct _aec_ref_op
{
    ...
    unsigned max_jitter;
    unsigned interm_idxs[];   /* buffer indexes for intermediate stage to avoid using input
                               * buffers as scratch pad. Input audio will be transferred to
                               * interim buffers, also any inserted silence will be written
                               * to these buffers.                               *
                               */
}cbops_aec_ref_op;

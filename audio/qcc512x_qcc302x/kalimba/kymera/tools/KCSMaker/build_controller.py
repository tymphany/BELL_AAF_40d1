    ...
    def write_makefile(self, cap_list, exe_name):
        ...
        for line in make_tmplt:
            if "EXTRA_INCDIRS +=" in line:
                ...
                # The *-group options are so that the libraries
                # freshly built for this bundle (in its CFG_LIBS) are
                # in a separate group from the base firmware, so that
                # those libraries can override copies in the base
                # firmware elf_name (they are always considered first
                # when trying to satisfy references).
                # It relies on the shape of build/compiler/kcc.mkf,
                # which puts all of LINK_EXTRA_FLAGS inside a
                # "--start-group ... --end-group".
                line = "export LINK_EXTRA_FLAGS = --end-group --start-group --just-symbols=" + self.convertPath(self.elf_name) + "\n"
